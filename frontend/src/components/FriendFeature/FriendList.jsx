...
export default function FriendList({
  ...
  onCopyUID,
  onSelectFriend,
  selectedFriendId: externalSelectedId,
}) {
  ...
  const fetchLastMessages = async (friendsList) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/messages/last-messages`,
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    const handleNewMessage = (msg) => {
      const otherUserId =
        msg.sender === currentUser.uid ? msg.receiver : msg.sender;
  ...
  const sortedFriends = [...friends].sort((a, b) => {
    ...
    // Fallback to alphabetical sorting with safety checks
