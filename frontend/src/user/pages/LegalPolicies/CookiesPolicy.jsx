// frontend/src/user/pages/CookiesPolicy.jsx
import { useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import { Cookie, ChevronLeft, Printer, Download, Menu, X } from "lucide-react";
...
export default function CookiesPolicy() {
  ...
  const policyData = t("cookies.sections", { returnObjects: true });
  const policyTitle = t("cookies.title");
  const lastUpdated = t("cookies.lastUpdated");
  ...
  // Render section with nested content
  const renderSection = (section) => {
    return (
      <div className="space-y-4">
        ...
        {/* Handle nested objects (cookie types, management) */}
        {Object.entries(section).map(([subKey, subValue]) => {
          if (["title", "content", "items"].includes(subKey)) return null;
          if (typeof subValue === "object" && subValue !== null) {
            return (
              <div key={subKey} className="ml-4 mt-4 p-4 bg-gray-50 rounded-lg">
                {subValue.title && (
                ...
                {subValue.description && (
                  <p className="text-gray-700 mb-2">{subValue.description}</p>
                )}
                {subValue.content && (
                  <p className="text-gray-700 mb-2">{subValue.content}</p>
                )}
                {subValue.examples && (
